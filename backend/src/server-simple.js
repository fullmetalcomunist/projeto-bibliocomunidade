...
app.post('/api/books/:id/emprestar', (req, res) => {
    ...
    console.log('📤 Dados recebidos no empréstimo:', { bookId, membroId });
    console.log('📚 Livros disponíveis:', livros.map(l => ({ id: l.id, titulo: l.titulo })));
    console.log('👥 Membros disponíveis:', membros.map(m => ({ id: m.id, nome: m.nome })));
    ...
    if (!membro) {
        console.log('❌ Membro não encontrado ID:', membroId);
        return res.status(404).json({ error: 'Membro não encontrado' });
    }
    
    if (livro.status !== 'disponivel') {
        console.log('❌ Livro não disponível:', livro.titulo, 'Status:', livro.status);
